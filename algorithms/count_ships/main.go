...
// Корабль — это прямой непрерывный горизонтальный или вертикальный отрезок ячеек
// со значением 1, не соприкасающийся с другими кораблями (классические правила «Морского боя»).
//
...
//
// ВАЖНО: при таком ограничении подсчёт «верхних левых» ячеек точно равен числу кораблей.
// Для произвольных связных фигур (например, Г- или П-образных) этот приём даёт неверный
// результат — там нужен обход в ширину/глубину (BFS/DFS) или система непересекающихся множеств.
//
...
func main() {
	...
	if err != nil {
		...
		// Ожидаемый результат: 5 кораблей
		// 1. (0,0)
		// 2. (0,3)-(0,4)
		// 3. (1,1)-(2,1)-(3,1)-(4,1)
		// 4. (2,3)-(2,4)
