...
// package clause is set by the generator from the source file; this line is only for parsing the template.
package generated
...
func (r {{ .EntityName }}Repository) Get({{ .PrimaryName }} {{ .PrimaryType}}) (*{{ .EntityName }}, error) {
    ...
    // First (в отличие от Find) возвращает ошибку, если запись не найдена.
    err := r.db.Where("{{ .PrimarySQLName }} = ?", {{ .PrimaryName }}).First(entity).Error
    if err != nil {
        return nil, err
    }
    return entity, nil
